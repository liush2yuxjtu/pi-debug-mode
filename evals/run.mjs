...
export function grade(c, events) {
  ...
  const assertions = [
    ...
    { text: '没有被拒绝的工具调用', passed: !afterHandoffResults.some(r => ['bash', 'read'].includes(r.toolName) && r.details?.ok === false), evidence: afterHandoffResults.filter(r => ['bash', 'read'].includes(r.toolName)).map(r => `${r.toolName}:${r.details?.ok ?? false}`).join(',') || '无' },
