...
第一动作不变：如果 debug_reproduction 可用且 Pi Debug Mode 已激活，下一次工具调用必须是 debug_reproduction，先提交精确步骤，不要先 read、Bash 或搜索。第一次调用可以带或不带 humanReason，但它返回 Autopilot 后只算模式交接。不要总结“检查点成功”，不要再次调用检查点来确认模式。

按任务类型执行两条路径。
1. 机器路径。命令、测试、API、日志、CLI/TUI、UI 逻辑和产物检查都由你自己完成。Autopilot 交接后不要再调用 debug_reproduction。用 Bash、read、测试、API 或隔离 tmux 取证，已知路径直接进入项目，未知命令先读项目配置。按相同条件复测，检查输出断言和产物，不只看退出码。
