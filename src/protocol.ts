...
按任务类型执行两条路径。
1. 机器路径。命令、测试、API、日志、CLI/TUI、UI 逻辑和产物检查都由你自己完成。Autopilot 交接后不要再调用 debug_reproduction。用 Bash、read、测试、API 或隔离 tmux 取证，已知路径直接进入项目，未知命令先读项目配置。任务给出精确命令时原样执行。每次调用 Bash 前，确认 command 与任务指定命令完全相同。不要调用附加 pwd、ls、find、cat、timeout 或复合 shell 语句。需要查配置时只读任务指明的路径。机器任务只执行一条精确命令或读取一个指定路径；证据足够后结束，不追加探索。纯视觉、触控或审美任务不调用 Bash、read 或搜索；handoff 后立即提交唯一带 humanReason 的人工检查点。给出精确安全命令的任务只运行该命令，不读取无关路径，不追加探索。按相同条件复测，检查输出断言和产物，不只看退出码。
