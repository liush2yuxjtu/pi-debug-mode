...
type FunnelState = { schema: 1; id: string; lastDay: string; firstSuccess: boolean; returned: boolean; week: string | null };
...
function utcWeek(now: number): string { const d = new Date(now); d.setUTCDate(d.getUTCDate() - ((d.getUTCDay() + 6) % 7)); return d.toISOString().slice(0, 10); }
...
function config(): { endpoint: URL } | undefined {
  ...
  if (truthy("PI_TELEMETRY_DISABLED") || truthy("DO_NOT_TRACK") || isCI()) return;
  try {
...
async function send(endpoint: URL, event: FunnelEvent, id: string): Promise<void> {
  try {
    await fetch(endpoint, { method: "POST", redirect: "error", headers: { "content-type": "application/json" }, signal: AbortSignal.timeout(500), body: JSON.stringify({ schema_version: 1, event, event_id: randomUUID(), anonymous_install_id: id, package: PACKAGE, version: VERSION, timestamp: new Date().toISOString(), os: process.platform, node_major: Number(process.versions.node.split(".")[0]), ci: false }) });
  } catch { /* at-most-once telemetry: no retries or offline spool */ }
}
async function record(success: boolean): Promise<void> {
  const configured = config(); if (!configured) return;
  const file = statePath(), lock = `${file}.lock`;
  let locked = false, temporary: string | undefined, state: FunnelState | undefined;
  ...
  try {
    ...
    const now = Date.now(), day = utcDay(now), week = utcWeek(now);
    if (!state) { state = { schema: 1, id: randomUUID(), lastDay: day, firstSuccess: false, returned: false, week: null }; events.push("first_install", "first_launch"); }
    else if (!state.returned && state.lastDay < day) { state.returned = true; events.push("returning_user"); }
    state.lastDay = day;
  ...
  const instrumented = new Proxy(pi, {
    get(target, property, receiver) {
      ...
      return (tool: any) => {
        ...
        return target.registerTool({ ...tool, async execute(...args: any[]) { const result = await execute(...args); if (result?.details?.outcome?.kind === "fixed") void enqueue(true); return result; } });
      };
    },
  }) as ExtensionAPI;
